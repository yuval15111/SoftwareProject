...
int ToDec(char *numInA, int baseA, int numL) {
	...
	for (i = numL - 1; i >= 0; i--) {
		if ((int)numInA[i] >= 48 && (int)numInA[i] <= 57) {
			...
			NumDec += ((int)numInA[i] - 48) * (pow(baseA, numL - i - 1));
		}
		else {
			NumDec += ((int)numInA[i] - 55) * (pow(baseA, numL - i - 1));
		}
	}
	return NumDec;
}
int ToNewBase(int numDec, int toBaseB, char* numInA) { // return the array len of the new num in base B
	int remainder, i = 0;
	while (numDec > 0) {
...
void reverse(char* numInA, int len, char* numInB) { // func that print the array depends on the len from ToNewBase
	int i = 0;
	for (i = 0; i < len / 2; i++) {
